...
static
const uint8_t gap_irk[16] = { 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0
};

static
const uint8_t gap_erk[16] = { 0xfe, 0xdc, 0xba, 0x09, 0x87, 0x65, 0x43, 0x21, 0xfe, 0xdc, 0xba, 0x09, 0x87, 0x65, 0x43, 0x21
};
...
static void gap_verify_connection_parameters(Gap *gap)
{
		...
		params->conn_int_max < gap->connection_params.conn_interval)
	{
		...
		if (aci_l2cap_connection_parameter_update_req(				gap->service.connection_handle,
				params->conn_int_min,
				params->conn_int_max,
				gap->connection_params.slave_latency,
				gap->connection_params.supervisor_timeout))
...
static void gap_init_svc(Gap *gap)
{
	...
	aci_gap_init(		GAP_PERIPHERAL_ROLE,
		0,
		strlen(name), &gap->service.gap_svc_handle, &gap->service.dev_name_char_handle, &gap->service.appearance_char_handle);
...
static void gap_advertise_start(GapState new_state)
{
	...
	static
	const uint16_t gap_appearance = 0x0000;	//GAP_APPEARANCE_UNKNOWN
	...
	const uint8_t choiceSelection[] = {
	  0x1E, 0xFF, 0x4C, 0x00, 0x07, 0x19, 0x05, 0x00, 0x55, 0x10, 0x00, 0x00, 0x01, 0x06, 0xf6, 0x85, 0xda, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	status = aci_gap_update_adv_data(0x1F, choiceSelection);
